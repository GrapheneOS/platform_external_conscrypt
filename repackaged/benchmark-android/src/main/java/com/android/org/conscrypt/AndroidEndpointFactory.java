...
public enum AndroidEndpointFactory implements EndpointFactory {
    @SuppressWarnings("unused") CONSCRYPT(newConscryptFactories(false)),
    ...
    public ClientEndpoint newClient(ChannelType channelType, int port, String[] protocols,
        ...
        return new ClientEndpoint(factories.clientFactory, channelType, port, protocols, ciphers);
    }

    @Override
    public ServerEndpoint newServer(ChannelType channelType, int messageSize, String[] protocols,
                                    String[] ciphers) throws IOException {
